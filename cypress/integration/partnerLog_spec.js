...
describe('Partner login page layout test', function() {
    beforeEach(()=>{
        cy.visit('http://localhost:3000/partnershipLogin?language=sr&option=register');
    })
  ...
  it('has all necessary fields', function(){
  	...
  	cy.url().should('eq', 'http://localhost:3000/partnershipLogin?language=sr&option=login');
  ...
  it('changes the language serbian and english', function(){
    ...
    cy.url().should('eq', 'http://localhost:3000/partnershipLogin?language=en&option=register');
  	...
  	cy.get('#loginRedirection').click();
  	cy.url().should('eq', 'http://localhost:3000/partnershipLogin?language=en&option=login');
  	cy.get('input[placeholder="Email"]');
    ...
    cy.get('input[placeholder="Email"]');
  	...
  	cy.url().should('eq', 'http://localhost:3000/partnershipLogin?language=sr&option=register');
  	cy.get('input[placeholder="Naziv lokala"]');
  	cy.get('input[placeholder="PIB"]');
  	cy.get('.logInput div').contains('Grad');
  	cy.get('input[placeholder="Kontakt osoba"]');
